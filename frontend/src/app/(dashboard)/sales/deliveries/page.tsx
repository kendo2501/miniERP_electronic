...
import { listDeliveries, markDelivered, markDeliveryFailed } from "@/lib/api/sales";
...
export default function DeliveriesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      listDeliveries({ page, limit: 20, status: statusFilter !== "all" ? statusFilter : undefined }).then((r) => r.data),
  ...
  const markMut = useMutation({
    ...
    onSuccess: () => {
      toast.success(t.deliveries.delivered);
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {data && (
              <span className="text-sm text-muted-foreground ml-auto">
                {data.total} {t.deliveries.totalDeliveries}
              </span>
            )}
          </div>
        </CardHeader>
                    ...
                    <tr key={d.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-6 py-3">
                        <div>
                          ...
                          {d.failureReason && (
                            <p className="text-xs text-muted-foreground mt-0.5 max-w-[120px] truncate" title={d.failureReason}>
                              {d.failureReason}
                            </p>
                          )}
                        </div>
                        ...
                        {canCreate && d.status === "PENDING" && (
                          <div className="flex items-center gap-1">
                            ...
                            <Button
                              ...
                              onClick={() => setFailDialog({ id: d.id, number: d.deliveryNumber })}
      ...
      <Dialog open={!!failDialog} onOpenChange={(v) => { if (!v) { setFailDialog(null); setFailReason(""); } }}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setFailDialog(null); setFailReason(""); }}>
              {t.common.cancel}
            </Button>
            <Button
              variant="destructive"
